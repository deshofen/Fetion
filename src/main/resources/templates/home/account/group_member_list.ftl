...
<#if from == 1>
	<#list accountGroupMemberList as accountGroupMember>
		<li class="media" style="border-bottom: 1px solid rgba(0, 0, 0, 0.03);">
			...
			<div class="custom-control custom-checkbox">
				<input type="checkbox" <#if base_account.id == accountGroupMember.member.id>disabled="disabled"</#if> class="custom-control-input reduce-group-user-select" value="${accountGroupMember.id}" id="user-group-reduce-${accountGroupMember.id}">
...
<#if from == 2>
	<#list accountGroupMemberList as accountGroupMember>
		...
		<li class="media" style="border-bottom: 1px solid rgba(0, 0, 0, 0.03);">
			...
			<#if accountGroupMember.meStatus=="1">
				<div class="custom-control" style="padding-top:16px;">
					<font color="green">已禁言</font>
				</div>
			<#else>
				<div class="custom-control custom-checkbox">
					<input type="checkbox" <#if base_account.id == accountGroupMember.member.id>disabled="disabled"</#if> class="custom-control-input reduce-mStatus-user-select" value="${accountGroupMember.member.id}" id="user-group-reduce-${accountGroupMember.member.id}">
					<label class="custom-control-label" for="user-group-reduce-${accountGroupMember.member.id}"></label>
				</div>
			</#if>
		</li>
	</#list>
	...
	<#list accountGroupMemberList as accountGroupMember>
		...
		<#if accountGroupMember.meStatus=="1">
			<li class="media" style="border-bottom: 1px solid rgba(0, 0, 0, 0.03);">
				...
				<div class="custom-control custom-checkbox">
					<input type="checkbox" <#if base_account.id == accountGroupMember.member.id>disabled="disabled"</#if> class="custom-control-input a-reduce-mStatus-user-select" value="${accountGroupMember.member.id}" id="user-group-reduce-${accountGroupMember.member.id}">
					<label class="custom-control-label" for="user-group-reduce-${accountGroupMember.member.id}"></label>
				</div>
			</li>
		</#if>
...
<script>
	$("#add-group-users-submit-btn").click(function(){
		...
		ajaxRequest('update_group_member','post',postData,function(data){
			...
			sendMsg(websocket,{msg:aids.toString(),toId:0,fromId:${base_account.id},chatType:'event',msgType:'refresh_friend_list'});
		},function(data){
			layer.msg(data.msg);
		});
	});
	...
	$("#reduce-mStatus-users-submit-btn").click(function(){
		var checkedUsers = $("input.reduce-mStatus-user-select[type='checkbox']:checked");
		if(checkedUsers.length < 1){
			layer.msg('请选择禁言的用户');
			return;
		}
		...
		var postData = {mids:aids.toString(),status:"1",accountGroupId:$("#focus-view-group-id").val()};
		ajaxRequest('jinyan','post',postData,function(data){
			layer.msg('禁言成功！');
	...
	$("#a-reduce-mStatus-users-submit-btn").click(function(){
		var checkedUsers = $("input.a-reduce-mStatus-user-select[type='checkbox']:checked");
		if(checkedUsers.length < 1){
			layer.msg('请选择要解除禁言的用户');
			return;
		}
		var aids = new Array();
		checkedUsers.each(function(i,e){
		...
		var postData = {mids:aids.toString(),status:"0",accountGroupId:$("#focus-view-group-id").val()};
		ajaxRequest('jinyan','post',postData,function(data){
			layer.msg('解除成功！');

...
function buildWebSocket(userid){
	...
	if ("WebSocket" in window){
       // 打开一个 web socket
       ws = new WebSocket("ws://"+window.location.host+"/webSocket/"+userid+ "/" + window.jsPublicKey.replace(/\//g,","));
       //成功建立连接
       ws.onopen = function(){
...
function sendMsg(ws,option){
	...
	let data = {
		data: aesUtil.encrypt(JSON.stringify(option), aesKey),//AES加密后的数据
...
//更新公共聊天室列表
function refreshGroupListMsg(option){
	//重新获取自己的公共聊天室列表
	getPubGroupList();
}

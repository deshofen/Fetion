...
function createChatTab(option,isAdd){
	// var html = '<div id="Achat-'+option.friendId+'-tab"><a class="nav-link single-chat" id="chat-'+option.friendId+'-tab" onclick="clickChatTab('+option.friendId+')" data-toggle="pill" href="#chat-first" role="tab" aria-controls="chat-first" aria-selected="false" data-id="'+option.friendId+'">';
	...
	// html += '<div class="top_chat"  data-toggle="tooltip" title="置顶" ><a href="javascript:void(0)" onclick="toTop('+option.friendId+')"  id="button-top"><i class="feather icon-corner-right-up"></i></a></div>';
	if(option.username.length > 7){
		option.username = option.username.substr(0,7)+'...';
	}
	...
	html +='<p>'
	html += '<span style="font-size: 14px;width: 200px;display: inline-table;" class="last-msg">'+option.lastMsg+'</span>';
	html += '<span style="font-size: 14px;position: relative;" class="top_chat"  data-toggle="tooltip" title="置顶" ><i onclick="toTop('+option.friendId+')" class="feather icon-corner-right-up"></i></span>';
	html += '<span style="font-size: 14px;position: relative;" class="bottom_chat"  data-toggle="tooltip" title="取消置顶" ><i onclick="toBottom('+option.friendId+')" class="feather icon-corner-right-down"></i></span>';
	html +='</p>'

	html += '</div></div></a>';
	// html += '</div></div><div class="top_chat"  data-toggle="tooltip" title="置顶" ><a href="javascript:void(0)" onclick="toTop('+option.friendId+')"  id="button-top"><i class="feather icon-corner-right-up"></i></a></div></a>';
	// html += '</div></div></a><span class="top_chat"  data-toggle="tooltip" title="置顶" style="position: relative;' +
	// 	'    left: 280px;' +
	// 	'    bottom: 30px;"><a href="javascript:void(0)" onclick="toTop('+option.friendId+')"  id="button-top"><i class="feather icon-corner-right-up"></i></a></span><span class="bottom_chat"  data-toggle="tooltip" title="取消置顶" style="    position: relative;' +
	// 	'    left: 280px;' +
	// 	'    bottom: 30px;"><a href="javascript:void(0)" onclick="toBottom('+option.friendId+')"  id="button-top"><i class="feather icon-corner-right-down"></i></a></span></div>';

	if(option.flag=='true'){
		$("#chat-list-tab-top").append(html);
	}else {
...
function createGroupChatTab(option,isAdd){
	// var html = '<div id="Agroup-chat-'+option.groupId+'-tab"><a class="nav-link group-chat" id="group-chat-'+option.groupId+'-tab" onclick="clickGroupChatTab('+option.groupId+')" data-toggle="pill" href="#chat-first" role="tab" aria-controls="chat-first" aria-selected="false" data-id="'+option.groupId+'">';
	...
	html += '<div class="media-body">';
	if(option.groupName.length > 7){
		option.groupName = option.groupName.substr(0,7)+'...';
	}
	...
	html +='<p>'
	html += '<span style="font-size: 14px;width: 200px;display: inline-table;" class="last-msg">'+option.lastMsg+'</span>';
	html += '<span style="font-size: 14px;position: relative;" class="top_chat"  data-toggle="tooltip" title="置顶" ><i onclick="toGroupTop('+option.groupId+')" class="feather icon-corner-right-up"></i></span>';
	html += '<span style="font-size: 14px;position: relative;" class="bottom_chat"  data-toggle="tooltip" title="取消置顶" ><i onclick="toGroupBottom('+option.groupId+')" class="feather icon-corner-right-down"></i></span>';
	html +='</p>'
	html += '</div></div></a>';
	// html += '</div></div></a><span title="置顶"  data-toggle="tooltip" class="top_chat" style="position: relative;' +
	// 	'    left: 280px;' +
	// 	'    bottom: 30px;"><a href="javascript:void(0)" onclick="toGroupTop('+option.groupId+')"  id="button-top"><i class="feather icon-corner-right-up"></i></a></span><span  data-toggle="tooltip" title="取消置顶" class="bottom_chat" style="position: relative;' +
	// 	'    left: 280px;' +
	// 	'    bottom: 30px;"><a href="javascript:void(0)" onclick="toGroupBottom('+option.groupId+')"  id="button-top"><i class="feather icon-corner-right-down"></i></a></span></div>';
	if(option.flag=='true'){
		$("#chat-list-tab-top").append(html);
	}else {
		$("#chat-list-tab").prepend(html);
...
function getALLChatTapInfoFromLocal(){
	var allChatTapInfo = localStorage.getItem('byx_chatonline_info_'+$("#current-online-user-id").val());
...
function setChatTapInfoToLocal(option){
	var chatInfo = localStorage.getItem('byx_chatonline_info_'+$("#current-online-user-id").val());
	if(chatInfo == null){
		chatInfo = {};
	}else{
	...
	localStorage.setItem('byx_chatonline_info_'+$("#current-online-user-id").val(),JSON.stringify(chatInfo));
}
//所有会话信息存入本地
function setAllChatTapInfoToLocal(allChatTapInfo){
	localStorage.setItem('byx_chatonline_info_'+$("#current-online-user-id").val(),JSON.stringify(allChatTapInfo));
}
//聊天室会话信息存入本地
...
function showLastMsgTips(lastMsg,friendId,type){
	...
	if(type == 'single'){
		$("#chat-"+friendId+"-tab").find("span.last-msg").text(lastMsg);
		$("#chat-"+friendId+"-tab").find("span.chat-timing").text(dateFormat("HH:MM", new Date()));
	}
	if(type == 'group'){
		$("#group-chat-"+friendId+"-tab").find("span.last-msg").text(lastMsg);
	...
	for(var i=0; i< msgList.length; i++){
		if(msgList[i].type == 'single'){
			if($("#search-chat-list-tab>a#chat-"+msgList[i].id+"-tab").length == 0){
				...
				search.find("span.last-msg").html(replaced);
				$("#search-chat-list-tab").append(search.prop('outerHTML'));
			}else{
				//表示已经存在
				var replaced = msgList[i].content.replace(new RegExp(text, 'ig'),'<font color="red">' + text + '</font>');
				$("#search-chat-list-tab>a#chat-"+msgList[i].id+"-tab").find("span.last-msg").html(replaced);
			}
		}
		if(msgList[i].type == 'group'){
			if($("#search-chat-list-tab>a#group-chat-"+msgList[i].id+"-tab").length == 0){
				...
				//表示已经存在
				var replaced = msgList[i].content.replace(new RegExp(text, 'ig'),'<font color="red">' + text + '</font>');
				$("#search-chat-list-tab>a#group-chat-"+msgList[i].id+"-tab").find("span.last-msg").html(replaced);
			}
		}
	}
}

...
function getChatTapInfoFromCookie(friendId){
	var chatTapInfo = $.JSONCookie('byx_chatonline_info');
	return chatTapInfo[friendId];
}
//从cookie中获取所有会话信息
function getALLChatTapInfoFromCookie(){
	var allChatTapInfo = $.JSONCookie('byx_chatonline_info');
	return allChatTapInfo;
}
//会话信息存入cookie
function setChatTapInfoToCookie(option){
	var chatInfo = $.JSONCookie('byx_chatonline_info');
	updateDataToJson(chatInfo,option.friendId,option);
	//console.log(chatInfo);
	$.JSONCookie('byx_chatonline_info',chatInfo,{ expires: 365 });
}
//发送文本消息

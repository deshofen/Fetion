...
import com.fetion.base.service.admin.MenuService;
import com.fetion.base.service.admin.OperaterLogService;
import com.fetion.base.service.admin.RoleService;
...
public class RoleController {
	...
	@ResponseBody
	public Result<Boolean> add(RoleReq role){
		//用统一验证实体方法验证是否合法
	...
	@ResponseBody
	public Result<Boolean> edit(RoleReq role){
		//用统一验证实体方法验证是否合法
		CodeMsg validate = ValidateEntityUtil.validate(role);
		if(validate.getCode() != CodeMsg.SUCCESS.getCode()){
			return Result.error(validate);
		...
		if(roleService.updateRole(existRole,role.getAuthorities()) == 0){

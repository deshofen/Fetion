...
<#if from == 1>
	<#list accountGroupMemberList as accountGroupMember>
		<a class="nav-link group-account" id="new-group-chat-${accountGroupMember.id}-tab" data-toggle="pill" href="#" role="tab" aria-selected="true" group-id="${accountGroupMember.id}" groupname="${accountGroupMember.name}" grouppeople="${accountGroupMember.curPeople}" grouppicture="${accountGroupMember.picture}" >
			<div class="media">
				<img class="align-self-center headPic" src="/photo/view?filename=${accountGroupMember.picture}" alt="User Image">
				<div class="media-body">
					<h5>${accountGroupMember.name}<sup style="color:red;">【聊天室】</sup></h5>
					<p>${accountGroupMember.info!"这聊天室人很懒，聊什么也没写！"}</p>
				</div>
			</div>
		</a>
	</#list>
...
<script>
	...
	$(".group-account").click(function(){
		$.get('view_pub_group_info',{id:$(this).attr('group-id')},function(data,status,xhr){

<#list friendList as friend>
	<#if from == 0>
		<a class="nav-link friend-account" id="new-chat-${friend.id}-tab" data-toggle="pill" href="#" role="tab" aria-selected="true" account-status="${friend.friendAccount.chatStatus}" account-id="${friend.friendAccount.id}" username="${friend.username}" sex="${friend.sex}" email="${friend.email!""}" mobile="${friend.mobile!""}" data-key="${friend.id}" data-msg-status="${friend.msgStatus}" data-status="${friend.status}" data-remark="${friend.remark!""}">
			<div class="media">
				<img class="align-self-center rounded-circle headPic" src="/photo/view?filename=${friend.headPic}" alt="User Image">
				<div class="media-body">
					<h5>${friend.remark!friend.username}</h5>
					<p>${friend.info!"此人很懒，什么也没写！"}</p>
				</div>
			</div>
		</a>
	<#else>
		<li class="media">
			<img class="align-self-center rounded-circle" src="/photo/view?filename=${friend.headPic}" alt="Generic placeholder image">
			<div class="media-body">
				<h5><span>${friend.remark!friend.username}</span></h5>
				<p>${friend.info!"此人很懒，什么也没写！"}</p>
			</div>
			<div class="custom-control custom-checkbox">
				<input type="checkbox" class="custom-control-input create-group-select-user" onclick="selectedUser()" id="user-${friend.id}-check" value="${friend.id}" username="${friend.username}" headPic="${friend.headPic}" aid="${friend.id}">
				<label class="custom-control-label" for="user-${friend.id}-check"></label>
			</div>
		</li>
	</#if>
</#list>
